...
void graph::findSourceEdgeSet(int a, int b) {
    ...
    sort(neighborsA.begin(), neighborsA.end());
    sort(neighborsB.begin(), neighborsB.end());
    set_intersection(neighborsA.begin(), neighborsA.end(),
...
void graph::addEdgeToNeighborList(int u, int v) {
    ...
    if (find(neighborsU.begin(), neighborsU.end(), v) == neighborsU.end()) {
        ...
        sort(neighborsU.begin(), neighborsU.end());
    }
...
void graph::findCommonNeighbors(){
    for (int a = 0; a < totalNode; ++a) {
        for (int i = 0; i < neighborList[a].size(); ++i) {
            ...
            // Ensure (a, b) is considered only once (a < b)
            if (a < b) {
                // Find common neighbors of `a` and `b`
                vector<int> commonNeighbors;
                vector<int> &neighborsA = neighborList[a];
                vector<int> &neighborsB = neighborList[b];
                // Use set intersection to find common neighbors
                set_intersection(
